...
<form action="index.php?btn_update" method="POST" enctype="multipart/form-data" id="update_hang_hoa">
    ...
    <select name="ma_loai" id="ma_loai">
        ...
        foreach ($loai_hang as $loai_hang) {
            extract($loai_hang);
            $selected = ($ma_loai == $hang_hoa_info['ma_loai']) ? 'selected' : '';
            echo '<option ' . $selected . ' value="' . $ma_loai . '">' . $ten_loai . '</option>';
        }
        ?>
    </select>
    <label for="ten_hh">Tên hàng hóa</label>
    <input type="text" name="ten_hh" id="ten_hh" required value="<?= $ten_hh ?>">
    <label for="ma_hh">Mã hàng hóa</label>
    <input type="text" name="ma_hh" id="ma_hh" readonly value="<?= $ma_hh ?>">
    <label for="up_hinh">Ảnh sản phẩm</label>
    ...
    <input type="number" name="don_gia" id="don_gia" value="<?= $don_gia ?>">
    <label for="giam_gia">Giảm giá (vnđ)</label>
    <input type="number" name="giam_gia" id="giam_gia" required value="<?= $giam_gia ?>">
    <label>Hàng đặc biệt?</label>
    <input type="radio" value="1" name="dac_biet" <?= $dac_biet ? 'checked' : '' ?>>
    <label>Đặc biệt</label>
    <input type="radio" value="0" name="dac_biet" <?= !$dac_biet ? 'checked' : '' ?>>
    <label>Bình thường</label>
    <label for="ngay_nhap">Ngày nhập</label>
    <input type="date" name="ngay_nhap" id="ngay_nhap" required value="<?= $ngay_nhap ?>">
    <label for="so_luot_xem">Số lượt xem</label>
    <input type="text" name="so_luot_xem" id="so_luot_xem" readonly required value="<?= $so_luot_xem ?>">
    <label for="mo_ta">Mô tả sản phẩm</label>
    <textarea id="txtarea" spellcheck="false" name="mo_ta" id="textareaExample" rows="3"><?= $mo_ta ?></textarea>
